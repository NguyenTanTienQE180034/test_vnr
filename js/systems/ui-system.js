(function () {
  ...
  function upgradeTowerType(state, towerType) {
    ...
    if (state.supplies < price) {
      App.Effects.addFloatingText(state, 24, 138, "Thieu tiep te", "#ff8e95");
    ...
    if (placed) {
      App.Effects.addFloatingText(state, placed.x - 8, placed.y - 22, "NANG CAP", "#80ffb1");
      state.selectedTowerId = placed.id;
    } else {
      App.Effects.addFloatingText(state, 26, 140, `Nang cap ${App.towersData[towerType].name} toan cuc`, "#80ffb1");
    }
  ...
  function upgradeTowerTypePointStat(state, towerType, statKey, label) {
    ...
    if (!spendCP(state, cpCost)) {
      ...
      if (any) {
        App.Effects.addFloatingText(state, any.x - 10, any.y - 24, "Thieu CP", "#ff8e95");
      } else {
        App.Effects.addFloatingText(state, 26, 158, "Thieu CP", "#ff8e95");
      }
    ...
    if (any) {
      ...
      App.Effects.addFloatingText(state, 26, 158, `+${label} toan cuc cho ${App.towersData[towerType].name}`, "#8fdfff");
    }
  ...
  function repairSelectedTower(state) {
    ...
    if (state.supplies < price) {
      App.Effects.addFloatingText(state, tower.x - 10, tower.y - 24, "Thieu tiep te", "#ff8e95");
      return { ok: false, reason: "insufficient-supplies", price };
    }
    state.supplies -= price;
    tower.repair(tower.maxHp * 0.45);
    App.Effects.addFloatingText(state, tower.x - 10, tower.y - 24, `Sua -${price}`, "#9cf6b4");
    return { ok: true, price };
  }
  ...
  function sellSelectedTower(state) {
    ...
    App.Effects.addFloatingText(state, tower.x - 14, tower.y - 24, "Da ban", "#85f5ac");
    return { ok: true };
  }
  ...
  function pointUpgradeSelectedTower(state, statKey) {
    ...
    const label = statKey === "damage" ? "SAT THUONG" : statKey === "hp" ? "HP" : "TOC BAN";
  ...
  function upgradeBase(state) {
    ...
    if (!spendCP(state, cpCost)) {
      App.Effects.addFloatingText(state, base.x - 20, base.y - 36, "Thieu CP", "#ff9ba8");
      return { ok: false, reason: "insufficient-cp", cpCost };
    }
    base.upgradeLevel();
    App.Effects.addFloatingText(state, base.x - 24, base.y - 36, "NANG CAP CAN CU", "#9ce6ff");
    return { ok: true, cpCost };
  }
  ...
  function boostBaseHp(state) {
    ...
    base.boostMaxHp();
    App.Effects.addFloatingText(state, base.x - 16, base.y - 36, "+HP TOI DA", "#9ff8b5");
  ...
  function healBase(state) {
    ...
    if (base.healUsedThisWave) {
      App.Effects.addFloatingText(state, base.x - 18, base.y - 36, "Wave nay da hoi roi", "#ff9ba8");
    ...
    App.Effects.addFloatingText(state, base.x - 12, base.y - 36, "+HOI MAU", "#7cff9e");
    return { ok: true, cpCost: cost };
  }
  ...
  function getTowerPanelModel(state) {
    ...
    const laserBeamCount = tower.type === "laser" ? (tower.level >= 3 ? 3 : tower.level === 2 ? 2 : 1) : 0;
    ...
    return {
      ...
      attackSpeed: (tower.attackSpeed * tower.activeMultipliers.speed).toFixed(2),
      projectileSpeed: Math.floor(tower.projectileSpeed),
      specialText: `${laserBeamCount > 0 ? `Tia x${laserBeamCount} | ` : ""}${tower.special.piercingShots ? `Xuyen x${Math.max(1, tower.special.pierceCount || 1)}` : "-"} ${
      ...
      buttons: {
        upgrade: {
          ...
          text: upgradePrice ? `Nang cap (${upgradePrice})` : "Da toi da",
        },
        repair: {
          enabled: tower.hp < tower.maxHp && state.supplies >= repairPrice,
        ...
        pointDamage: {
          enabled: canPointUpgradeTowerType(state, tower.type, "damage") && state.commandPoints >= dmgCost,
          text: canPointUpgradeTowerType(state, tower.type, "damage") ? `+SAT THUONG (${dmgCost} CP)` : "+SAT THUONG (Lv3)",
        },
        pointHp: {
          enabled: canPointUpgradeTowerType(state, tower.type, "hp") && state.commandPoints >= hpCost,
          text: canPointUpgradeTowerType(state, tower.type, "hp") ? `+HP (${hpCost} CP)` : "+HP (Lv3)",
        },
        pointSpeed: {
          enabled: canPointUpgradeTowerType(state, tower.type, "speed") && state.commandPoints >= spdCost,
          text: canPointUpgradeTowerType(state, tower.type, "speed") ? `+TOC BAN (${spdCost} CP)` : "+TOC BAN (Lv3)",
        },
      },
    };
  }
  ...
  function getBasePanelModel(state) {
    ...
    return {
      ...
      buttons: {
        upgrade: {
          enabled: base.canUpgradeLevel() && state.commandPoints >= (levelUpCost || 0),
          text: base.canUpgradeLevel() ? `Nang cap can cu (${levelUpCost} CP)` : "Can cu toi da",
        },
        hpBoost: {
        ...
        heal: {
          enabled: !base.healUsedThisWave && state.commandPoints >= healCost && base.hp < base.maxHp,
          text: base.healUsedThisWave ? "Hoi mau da dung wave nay" : `Hoi mau (${healCost} CP)`,
        },
      },
    };
  }
  ...
  function getWaveStatusText(state) {
    ...
    if (state.wavePhase === "combat") {
      const bossStatus = state.bossSpawnedInWave ? (state.bossKilledInWave ? "Boss da ha" : "Dang danh boss") : "Boss chua xuat hien";
      return `Wave ${state.wave} | Con ${Math.ceil(state.waveTimer)}s | ${bossStatus}`;
    }
  ...
  function getHudModel(state) {
    return {
      ...
      wave: `${state.wave}`,
      level: "VO HAN",
      enemiesLeft: `${state.enemies.filter((e) => e.hp > 0).length + state.spawnQueue.length}`,
      waveState: getWaveStatusText(state),
      quizCd: state.quizCooldown > 0 ? `Quiz hoi sau ${state.quizCooldown.toFixed(1)}s` : "Quiz san sang",
      paused: state.paused,
      bossWarningVisible: state.boss.warningTimer > 0,
      bossHealthVisible: state.boss.healthBarVisible && !!state.boss.activeId,
    };
  }
  ...
  function getResultModel(state) {
    return {
      title: state.result.state === "gameOver" ? "That bai" : "Ket thuc",
